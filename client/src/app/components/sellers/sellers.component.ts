...
export class SellersComponent implements OnInit {
  ...
  getOptionPremiums(): void {
    ...
    console.log(
      'Number of sellers selling: ' + __this.sellers.length
    );

    // tslint:disable-next-line:prefer-const
    let optionPremiumEvents = [];

    for (let i = 0; i < __this.sellers.length; i++) {
      ...
      optionPremiumEvents.push(
        __this.contractService.optionFactories[i].OptionPremium(function(
          ...
          console.log('Option premium from factory ' + i + ': ' + __this.sellers[i][3]);
        })
      );
    }
  }
  ...
  buyOption(): void {
    ...
    for (let i = 0; i < __this.sellers.length; i++) {
      ...
      if (
        __this.sellers[i][3] <
        __this.sellers[__this.contractService.selectedOptionFactoryId][3]
      ) {
    ...
    console.log('Cheapest premium: ' + __this.sellers[__this.contractService.selectedOptionFactoryId][3]);
    ...
    const amount = __this.contractService.web3.toWei(
      (__this.sellers[__this.contractService.selectedOptionFactoryId][3] * 1.05) /
  ...
  private saveOptionPremium(): void {
    localStorage.setItem(
      'optionPremium',
      JSON.stringify(this.sellers[0].optionPremium)
    );
  }
